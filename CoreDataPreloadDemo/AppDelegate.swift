...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func parseCSV (contentsOfURL: NSURL, encoding: NSStringEncoding, error: NSErrorPointer) -> [(name:String, detail:String, price: String)]? {
        ...
        if let content: NSString? = try? NSString(contentsOfURL: contentsOfURL, encoding: encoding) {
            items = []
            print(content)
            ...
            if let items = parseCSV(contentsOfURL, encoding: NSUTF8StringEncoding, error: &error) {
                ...
                if let managedObjectContext = self.managedObjectContext as NSManagedObjectContext? {
                    for item in items {
                        ...
                        if((try? self.managedObjectContext.save()) != nil) {
                        }
                    }
                }
            }
    ...
    func removeData () {
        ...
        if let managedObjectContext = self.managedObjectContext as NSManagedObjectContext? {
            ...
            if let menuItems = try? managedObjectContext.executeFetchRequest(fetchRequest) as! [MenuItem] {
